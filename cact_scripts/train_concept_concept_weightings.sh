...
BASE_JOB_NAME="concept_act_so101_conceptweight"
...
CONCEPT_WEIGHTINGS=(0.001 0.1 0.2 0.5 10.0)
...
for SEED in "${SEEDS[@]}"; do
  for PERCENT in "${PERCENTS[@]}"; do
    for CONCEPT_WEIGHT in "${CONCEPT_WEIGHTINGS[@]}"; do
      echo "Starting training with seed $SEED and $CONCEPT_WEIGHTINGS"

      # Update job name and output directory to include seed
      JOB_NAME="${BASE_JOB_NAME}_weight_${CONCEPT_WEIGHT}_seed_${SEED}"
      OUTPUT_DIR="${BASE_OUTPUT_DIR}_weight_${CONCEPT_WEIGHT}_seed_${SEED}"
      ...
      python lerobot/scripts/train.py \
        ...
        echo "Completed training with seed $SEED and $CONCEPT_WEIGHT"
        done
    done
  done
